...
const PERSONS_URL = `${BASE}/members/persons.json`;
const MEMBERS_URL = `${BASE}/members/members.json`;
...
export default function Page() {
  const [personsMap, setPersonsMap] = useState<PersonsMap>({});
  ...
  useEffect(() => {
    async function load() {
      try {
        const [pRes, mRes, lRes, dRes] = await Promise.all([
          fetch(PERSONS_URL, { cache: "no-store" }),
          fetch(MEMBERS_URL, { cache: "no-store" }),
          fetch(LOGS, { cache: "no-store" }),
          fetch(DEVICES, { cache: "no-store" }),
        ]);

        setPersonsMap((await pRes.json()) || {});
        setMembersMap((await mRes.json()) || {});
  ...
  const siteList = useMemo(() => {
    ...
    Object.entries(devices).forEach(([devId, info]) => {
      ...
      if (!sites.has(siteId)) sites.set(siteId, { id: siteId, name: siteName, devices: [] });
  ...
  const filteredLogs = useMemo(() => {
    if (selectedSite === "all") {
      const testDevices = siteList.filter(s => s.id === "test").flatMap(s => s.devices);
  ...
  useEffect(() => {
    ...
    fetch(LATEST, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).catch(() => {});
  }, [latestList]);
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {siteList.map((site) => (
              <option key={site.id} value={site.id}>{site.name} ({site.devices.length}台)</option>
            ))}
          </select>
        ...
        <button onClick={() => location.reload()} className="px-3 py-1.5 rounded-full bg-gray-900 text-white text-sm shadow">再読込</button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <section className="lg:col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {latestList.map((r) => {
              const p = getPersonByUid(r.uid);
              const inState = r.state === 1;
              return (
                <div key={r.uid} className={`relative bg-white border rounded-2xl shadow-sm p-3 ${companyAccent(p.company)} border-l-4`}>
                  <div className="flex items-center justify-between">
                    <div className="text-[15px] font-semibold">{p.name || r.uid}</div>
        ...
        <aside className="lg:col-span-1">
          <div className="flex items-center justify-between mb-2">
            ...
            <label className="text-xs text-gray-600 flex items-center gap-2">
              件数:
              <select className="border rounded-md px-2 py-1 text-xs" value={logCount} onChange={(e) => setLogCount(Number(e.target.value))}>
                ...
                {logsWithState.map((r) => {
                  ...
                  return (
                    <tr key={r.id} className="[&>td]:px-3 [&>td]:py-2 border-b last:border-b-0">
                      ...
                      <td className="font-medium">{p.name || r.uid}</td>
                      <td>
